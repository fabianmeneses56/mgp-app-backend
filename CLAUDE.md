...
The app auto-deploys on every push to `main` via GitHub Actions (`.github/workflows/deploy.yml`): a `ci` job (lint, build, test) must pass, then the `deploy` job connects to the VPS over Tailscale/SSH and runs `deploy/deploy.sh`, which `git pull`s `main` and runs `docker compose --env-file .env.production -f docker-compose.prod.yml up -d --build`. `docker-compose.prod.yml` defines three services: `db` (Postgres, local to the VPS, no managed DB), `app` (built from the root `Dockerfile`, `CMD ["node", "dist/main"]`), and `caddy` (reverse proxy/TLS termination, fronting `app` on 80/443). Production sets `NODE_ENV=production` (hardcoded in the `Dockerfile`), its own `JWT_SECRET` (different from the local `.env` one), `DB_SSL=false` (the VPS Postgres container has no TLS listener, unlike a managed DB — see `database.factory.ts`), and `CORS_ORIGIN` left empty since the only consumer is a React Native/Expo app (CORS is a browser-only mechanism, irrelevant to native clients). Config lives in `.env.production` on the VPS (gitignored; `.env.production.example` is the template).
